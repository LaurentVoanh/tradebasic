...
async function fetchAll(){try{const[s,c,a,l]=await Promise.all([fetch('api.php?action=stats'),fetch('api.php?action=coins'),fetch('api.php?action=agents'),fetch('api.php?action=logs&limit=50')]);updateStats(await s.json());updateMarket(await c.json());updateAgents(await a.json());updateConsole(await l.json());}catch(e){console.error(e)}}
